...
class LeaveRequest extends FormRequest
{
    ...
    public function messages(): array
    {
        return [
            ...
            'hours.required' => 'Le ore del congedo sono obbligatorie.',
            ...
            'motivation_custom.required' => 'La motivazione custom e obbligatoria quando selezioni Altro.',
        ...
        $validator->after(function (Validator $validator) {
            ...
            if ($overlappingLeaves) {
                $validator->errors()->add(
                    'start_date',
                    'Hai gia una richiesta congedo su uno o piu giorni selezionati.'
                );
            }
            ...
            $requiredNoticeWorkingHours = Leave::requestNoticeWorkingHours();
            if ($requiredNoticeWorkingHours > 0) {
                $requestedStartDateTime = Leave::resolveRequestedStartDateTime($startDate, $lessonsStart);
                $availableWorkingMinutes = Leave::workingMinutesBetween(now(), $requestedStartDateTime);

                if ($availableWorkingMinutes < ($requiredNoticeWorkingHours * 60)) {
                    $validator->errors()->add(
                        'start_date',
                        'La richiesta di congedo deve essere inviata almeno '
                        .$requiredNoticeWorkingHours
                        .' ore lavorative prima dell inizio previsto (weekend e vacanze scolastiche esclusi).'
                    );
                }
            }
