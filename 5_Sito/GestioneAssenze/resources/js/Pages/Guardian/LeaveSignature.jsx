...
export default function LeaveSignature({
    token,
    status,
    ...
    const submit = (event) => {
        ...
        post(route('guardian.leaves.signature.store', { token }), {
            preserveScroll: true,
            onFinish: () => {
    ...
    return (
        <>
            <Head title="Firma congedo tutore" />
            ...
            <div className="min-h-screen bg-slate-100 px-3 py-4 text-slate-900 sm:px-6 sm:py-8">
                <main className="mx-auto w-full max-w-4xl rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                    <h1 className="text-xl font-semibold text-slate-900">
                        Conferma congedo e firma tutore
                    </h1>
                    ...
                    {signatureSuccess && (
                        <div className="mt-4 rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-2 text-sm text-emerald-700">
                            Firma registrata con successo. Le decisioni gia prese dalla scuola non vengono modificate.
                        </div>
                    )}

                    {status === 'expired' && (
                        <div className="mt-4 rounded-lg border border-rose-300 bg-rose-50 px-3 py-2 text-sm text-rose-700">
                            Il link e scaduto. Chiedi il reinvio della richiesta firma.
                        </div>
                    )}

                    {status === 'used' && (
                        <div className="mt-4 rounded-lg border border-blue-300 bg-blue-50 px-3 py-2 text-sm text-blue-700">
                            Questo link e gia stato utilizzato. Se serve, richiedi un nuovo invio.
                        </div>
                    )}

                    {status === 'invalid' && (
                        <div className="mt-4 rounded-lg border border-rose-300 bg-rose-50 px-3 py-2 text-sm text-rose-700">
                            Link non valido. Verifica il collegamento ricevuto via email.
                        </div>
                    )}

                    {alreadySigned && (
                        <div className="mt-4 rounded-lg border border-blue-300 bg-blue-50 px-3 py-2 text-sm text-blue-700">
                            Questo congedo risulta gia firmato
                    ...
                    {canSign && (
                        <form onSubmit={submit} className="mt-5 space-y-5">
                            ...
                            <div className="rounded-lg border border-slate-200 bg-white p-3 sm:p-4">
                                ...
                                <div className="mt-2 flex flex-col gap-2 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                                    <span className="leading-relaxed">
                                        Firma all interno del riquadro prima di inviare.
                                    </span>
                                    <button
                                        type="button"
                            ...
                            <label className="flex items-start gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
                                ...
                                />
                                <span>
                                    Dichiaro di aver verificato i dati e confermo il congedo in qualita di tutore legale.
                                </span>
                            </label>
                            {errors.consent && (
                                <p className="-mt-2 text-xs text-rose-600">
